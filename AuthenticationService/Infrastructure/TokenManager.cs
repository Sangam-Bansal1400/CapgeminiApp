...
namespace AuthenticationService.Infrastructure
{
    public class TokenManager
    {
        ...
        public TokenManager(
            ...
            IOptions<AppSettings> appSettings) => (_repository, _appSettings) = (repository, appSettings.Value);
    
    public string GetJwtToken(AuthenticationRequest request)
        {
            if (request is null)
            {
            ...
            var signingCred = new SigningCredentials(
                key: new SymmetricSecurityKey(secretKey), algorithm: SecurityAlgorithms.HmacSha256Signature);
        ...
        {
            var secetkey=Encoding.UTF8.GetBytes(_appSettings.AppSecret);
            SymmetricSecurityKey key = new SymmetricSecurityKey(secetkey);
